...
const { InsaEduCrawling, InsaHrCrawling } = require("./insa-crawler");

async function insaEduUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "insaEdu.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous insaEdu not found");
  }

  const newsCralwer = await InsaEduCrawling();
  ...
  if (_.isEqual(newData, prevData)) {
    console.log("insaEdu has not been changed");
    return;
  }
  ...
  console.log("insaEdu updated successfully");
}

async function insaHrUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "insaHr.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous insaHr not found");
  }

  const newsCralwer = await InsaHrCrawling();
  ...
  // 변경된 값이 없으면 아무 것도 하지 않음
  if (_.isEqual(newData, prevData)) {
    console.log("insaHr has not been changed");
    return;
  }
  ...
  console.log("insaHr updated successfully");
}

module.exports = { insaEduUpdate, insaHrUpdate };
