...
const { KeeaAnnCrawling, KeeaRawCrawling } = require("./keea-crawler");

async function keeaAnnUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "keeaAnn.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous keeaAnn not found");
  }

  const newsCralwer = await KeeaAnnCrawling();
  ...
  if (_.isEqual(newData, prevData)) {
    console.log("keeaAnn has not been changed");
    return;
  }
  ...
  console.log("keeaAnn updated successfully");
}

async function keeaRawUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "keeaRaw.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous keeaRaw not found");
  }

  const newsCralwer = await KeeaRawCrawling();
  ...
  // 변경된 값이 없으면 아무 것도 하지 않음
  if (_.isEqual(newData, prevData)) {
    console.log("keeaRaw has not been changed");
    return;
  }
  ...
  console.log("keeaRaw updated successfully");
}

module.exports = { keeaAnnUpdate, keeaRawUpdate };
