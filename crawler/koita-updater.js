...
const { KoitaNotCrawling } = require("./koita-crawler");

async function koitaNotUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "koitaNot.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous koitaNot not found");
  }

  const newsCralwer = await KoitaNotCrawling();
  ...
  if (_.isEqual(newData, prevData)) {
    console.log("koitaNot has not been changed");
    return;
  }
  ...
  await apiClient.updateKoitaNot({
    nTitle,
    nDate,
    nLink,
  });
  ...
  console.log("koitaNot updated successfully");
}

module.exports = { koitaNotUpdate };