...
const { LawmkOpCrawling } = require("./lawmk-crawler");

async function lawmkOpUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "lawmkOp.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous lawmkOp not found");
  }

  const newsCralwer = await LawmkOpCrawling();
  ...
  if (_.isEqual(newData, prevData)) {
    console.log("lawmkOp has not been changed");
    return;
  }
  ...
  console.log("lawmkOp updated successfully");
}

module.exports = { lawmkOpUpdate };