...
const { AbouthrMgCrawling, AbouthrPrCrawling, AbouthrRcCrawling } = require("./abouthr-crawler");

async function abouthrMgUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "abouthrMg.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous abouthrMg not found");
  }

  const newsCralwer = await AbouthrMgCrawling();
  ...
  if (_.isEqual(newData, prevData)) {
    console.log("abouthrMg has not been changed");
    return;
  }
  ...
  await apiClient.updateAbouthrMg({
    nTitle,
    nDate,
    nLink,
  });
  ...
  console.log("abouthrMg updated successfully");
}

async function abouthrPrUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "abouthrPr.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous abouthrPr not found");
  }

  const newsCralwer = await AbouthrPrCrawling();
  ...
  // 변경된 값이 없으면 아무 것도 하지 않음
  if (_.isEqual(newData, prevData)) {
    console.log("abouthrPr has not been changed");
    return;
  }
  ...
  await apiClient.updateAbouthrPr({
    nTitle,
    nDate,
    nLink,
  });
  ...
  console.log("abouthrPr updated successfully");
}

async function abouthrRcUpdate(outputPath, apiClient) {
  let prevData = {};
  const newsListPath = path.join(outputPath, "abouthrRc.json");
  try {
    // 기존 크롤링한 값이 있다면 불러오기
    prevData = JSON.parse(fs.readFileSync(newsListPath, "utf-8"));
  } catch (e) {
    console.log("previous abouthrRc not found");
  }

  const newsCralwer = await AbouthrRcCrawling();
  ...
  // 변경된 값이 없으면 아무 것도 하지 않음
  if (_.isEqual(newData, prevData)) {
    console.log("abouthrRc has not been changed");
    return;
  }
  ...
  await apiClient.updateAbouthrRc({
    nTitle,
    nDate,
    nLink,
  });
  ...
  console.log("abouthrRc updated successfully");
}

module.exports = { abouthrMgUpdate, abouthrPrUpdate, abouthrRcUpdate };
