...
async function getRocm() {
  const apiClient = axios.create({
    baseURL: process.env.CB_API_BASE_URL || "http://localhost:8080",
  });

  const response = await apiClient.get("sRocm");
  ...
  const response2 = await apiClient2.get("wRocm");
  ...
  const response3 = await apiClient3.get("dRocm");
  ...
  const response4 = await apiClient4.get("bRocm");
  ...
  const response5 = await apiClient5.get("iRocm");
