...
async function getKld() {
  const apiClient = axios.create({
    baseURL: process.env.CB_API_BASE_URL || "http://localhost:8080",
  });

  const response = await apiClient.get("kldCon");
  ...
  const response2 = await apiClient2.get("kldCm");
  ...
  const response3 = await apiClient3.get("kldTech");
  ...
  const response4 = await apiClient4.get("kldSafety");
