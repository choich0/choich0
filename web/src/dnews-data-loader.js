...
async function getDnews() {
  const apiClient = axios.create({
    baseURL: process.env.CB_API_BASE_URL || "http://localhost:8080",
  });

  const response = await apiClient.get("dnewsStd");
  ...
  const response2 = await apiClient2.get("dnewsEng");
  ...
  const response3 = await apiClient3.get("dnewsTech");
  ...
  const response4 = await apiClient4.get("dnewsMat");
  ...
  const response5 = await apiClient5.get("dnewsSafety");
