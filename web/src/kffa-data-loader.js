...
async function getKffa() {
  const apiClient = axios.create({
    baseURL: process.env.CB_API_BASE_URL || "http://localhost:8080",
  });

  const response = await apiClient.get("kffaAnn");
  ...
  const response2 = await apiClient2.get("kffaNot");
  ...
  const response3 = await apiClient3.get("kffaRaw");
